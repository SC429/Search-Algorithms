...
public class MyLinkedList
{
  ...
  class Node
  {
   ...
   // TODO: Sort this linked list backwards using merge sort (recursively)
   public Node sort(Node node) {
	   if (node.next == null) {
		   return node;
	   }
	   Node midRef = getMiddle(node);
	   Node leftLast = midRef;
	   Node rightFirst = midRef.next;
	   midRef.next = null;
	   
	   Node leftHead =  sort(node);
	   Node rightHead = sort(rightFirst);
	   return merge(leftHead, rightHead);
   }

   // TODO: Merge two sorted lists (can be non-recursive)
   public Node merge(Node left, Node right) {   // Backwards implementation
